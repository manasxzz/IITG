...
# ---------- Fit Model with Error Estimation ----------
def fit_r2_data(freq, r2_eff):
    p0 = [np.min(r2_eff), 100.0, 0.01]
    bounds = ([0, 1, 0], [np.inf, 1e5, 1])
    try:
        popt, pcov = curve_fit(model_wrapper, freq, r2_eff, p0=p0, bounds=bounds, maxfev=10000)
...
# ---------- Plot + Table with Error Bars and Parameter Errors ----------
def plot_all_fits(datasets):
    ...
    for label, (freq, r2) in datasets.items():
        ...
        # Assume a relative uncertainty of 5% for R2_eff data points if no error data provided
        r2_err = 0.05 * r2
        plt.errorbar(freq, r2, yerr=r2_err, fmt='o', label=f"{label} Data", capsize=3, markersize=5)

        results.append({
            "Residue": label,
            "R2 (1/s)": round(popt[0], 3),
            "R2 Error (1/s)": round(perr[0], 3),
            "kex (1/s)": round(popt[1], 3),
            "kex Error (1/s)": round(perr[1], 3),
            "phi": round(popt[2], 6),
            "phi Error": round(perr[2], 6)
        })

    plt.xlabel("ν_CPMG (Hz)")
    plt.ylabel("R₂_eff (1/s)")
    plt.title("CPMG Relaxation Dispersion Fits with Error Bars")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.show()

    # Output table with errors
    print("\n📋 Fitted Parameters with Errors:")
    results_df = pd.DataFrame(results)
    print(results_df.to_string(index=False))

    # Save to CSV
    results_df.to_csv("fitted_parameters_with_errors.csv", index=False)
    print("\n✅ Saved to fitted_parameters_with_errors.csv")
