...
def R2eff_model(v_cpmg, R2, k_AB, k_BA, delta_ppm):
    ...
    D_minus = 0.5 * (-1 + (psi + 2 * delta ** 2) / np.sqrt(psi ** 2 + zeta ** 2))
...
# Load Excel file
file_path = 'r2results.xlsx'  # Change this to your actual file path
...
# Plot
plt.errorbar(v_cpmg, R2eff_exp, yerr=R2eff_err, fmt='o', label='Experimental', capsize=4)
